...
var isDist = __dirname.endsWith(path.join("dist"));
var rootDir = isDist ? path.resolve(__dirname, "..") : __dirname;
var COMMANDS_DIR = path.join(rootDir, "commands", "kiro");
var HELPERS_DIR = path.join(rootDir, "skills", "kiro-skill", "helpers");
var SPECS_BASE = ".kiro/specs";
...
function parseTasksFile(content) {
  ...
  for (const line of lines) {
    ...
    if (!match) continue;
    const [, statusChar, id, content2] = match;
    const status = statusChar === "x" ? "done" : statusChar === "-" ? "in_progress" : "pending";
    const reqMatch = content2.match(/_Requirements?: ([\d., ]+)_/);
    const requirements = reqMatch ? reqMatch[1].split(/,\s*/).map((r) => r.trim()) : [];
    const isSubtask = id.includes(".");
    const parentId = isSubtask ? id.substring(0, id.lastIndexOf(".")) : void 0;
    tasks.push({
      id,
      status,
      content: content2.replace(/_Requirements?: [\d., ]+_/, "").trim(),
      subtasks: [],
      requirements,
      parentId
    });
...
async function kiroStatus() {
  ...
  const features = fs.readdirSync(SPECS_BASE).filter(
    (f) => fs.statSync(path.join(SPECS_BASE, f)).isDirectory()
  );
