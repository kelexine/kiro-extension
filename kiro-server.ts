...
type Phase = "spec" | "design" | "task" | "execute";
...
function getSpecPath(feature: string, file: string): string {
  return path.join(SPECS_BASE, feature, file);
}

function loadState(feature: string): State | null {
  const statePath = path.join(SPECS_BASE, feature, "state.json");
  if (!fileExists(statePath)) return null;
  
  try {
    return JSON.parse(readFile(statePath)) as State;
  } catch {
    return null;
  }
}

function saveState(feature: string, state: State): void {
  const statePath = path.join(SPECS_BASE, feature, "state.json");
...
// Task parsing
function parseTasksFile(content: string): Task[] {
  const tasks: Task[] = [];
...
function validateTaskSequence(tasks: Task[], taskId: string, newStatus: TaskStatus): string | null {
  ...
  */
  ...
  if (task.requirements.length > 0 && newStatus !== "pending") {
    const unmetReqs = task.requirements.filter(
      reqId => !tasks.find(t => t.id === reqId && t.status === "done")
    );
...
async function kiroDesign(feature: string): Promise<string> {
  ...
  // Update state
  const state: State = loadState(feature) || {
    feature,
    phase: "spec",
    completed_tasks: [],
    last_updated: "",
...
async function kiroTask(feature: string): Promise<string> {
...
async function kiroExecute(feature: string, taskId?: string): Promise<string> {
...
async function kiroVibe(): Promise<string> {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    switch (name) {
      case "kiro_spec":
        return { content: [{ type: "text", text: await kiroSpec(args.feature as string) }] };
      case "kiro_design":
        return { content: [{ type: "text", text: await kiroDesign(args.feature as string) }] };
      case "kiro_task":
        return { content: [{ type: "text", text: await kiroTask(args.feature as string) }] };
      case "kiro_execute":
        return {
      ...
      case "get_task":
        return {
          content: [{ type: "text", text: await getTask(args.feature as string, args.task_id as string) }],
        };
      case "set_task":
        return {
          content: [
            { type: "text", text: await setTask(args.feature as string, args.task_id as string, args.status as TaskStatus) },
          ],
        };
      default:
